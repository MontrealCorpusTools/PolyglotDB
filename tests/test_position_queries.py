...
def test_initial_query(acoustic_config):
    with CorpusContext(acoustic_config) as g:
        ...
        q = q.columns(
            g.word.following.phone.initial.label.column_name("initial_following_phone"),
            g.word.following.phone.initial.duration.column_name(
                "initial_following_phone_duration"
            ),
...
def test_final_query(acoustic_config):
    with CorpusContext(acoustic_config) as g:
        ...
        q = q.columns(
            g.word.previous.phone.final.label.column_name("final_previous_phone"),
...
def test_penult_query(acoustic_config):
    with CorpusContext(acoustic_config) as g:
        q = g.query_graph(g.word).filter(g.word.label == "is")
        q = q.columns(
            g.word.previous.phone.penultimate.label.column_name("phone"),
        ...
        q = g.query_graph(g.word).filter(g.word.label == "is")
        q = q.columns(
            g.word.previous.phone.antepenultimate.label.column_name("phone"),
