...
def test_basic_query(graph_db):
    with CorpusContext(corpus_name = 'untimed', **graph_db) as g:
        ...
        assert(all(x.r['label'] == 'are' for x in q.all()))

def test_order_by(graph_db):
    with CorpusContext(corpus_name = 'timed', **graph_db) as g:
        q = g.query('word').filter(Annotation.label == 'are').order_by(Annotation.begin).times()
        prev = 0
        for x in q.all():
            assert(x.begin > prev)
            prev = x.begin
...
def test_query_word_in(graph_db):
    with CorpusContext(corpus_name = 'untimed', **graph_db) as g:
        q = g.query('phone').filter(Annotation.label == 'k')
        q = q.filter_contained_by('word', Annotation.label.in_(['cats','dogs','cute']))
        print(q.cypher())
...
def test_query_aggregate_count(graph_db):
    with CorpusContext(corpus_name = 'timed', **graph_db) as g:
        q = g.query('phone').filter(Annotation.label == 'aa').count()
        assert(q == 3)
