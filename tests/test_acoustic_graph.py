...
def test_query_duration(graph_db):
    with CorpusContext(corpus_name = 'acoustic', **graph_db) as g:
        q = g.query('phone').filter(Annotation.label == 'aa').order_by(Annotation.begin).duration()
        results = q.all()
...
def test_query_duration_aggregate_average(graph_db):
    with CorpusContext(corpus_name = 'acoustic', **graph_db) as g:
        ...
        results = q.aggregate(Average('duration'))
        print(results)
        assert(abs(results[0].average_duration - 0.08) < 0.001)

def test_query_count_group_by(graph_db):
    with CorpusContext(corpus_name = 'acoustic', **graph_db) as g:
        q = g.query('phone').filter(Annotation.label.in_(['aa','ae']))
        results = q.group_by(Annotation.label).aggregate(Count('*'))
        assert(len(results) == 2)
        assert(results[0].label == 'ae')
        assert(results[0].count_all == 7)

        assert(results[1].label == 'aa')
        assert(results[1].count_all == 3)

def test_query_duration_aggregate_average_group_by(graph_db):
    with CorpusContext(corpus_name = 'acoustic', **graph_db) as g:
        q = g.query('phone').filter(Annotation.label.in_(['aa','ae']))
        results = q.group_by(Annotation.label).aggregate(Average('duration'))

        assert(len(results) == 2)
        assert(results[0].label == 'ae')
        assert(abs(results[0].average_duration - 0.193) < 0.001)

        assert(results[1].label == 'aa')
        assert(abs(results[1].average_duration - 0.08) < 0.001)

@pytest.mark.xfail
def test_query_pitch(graph_db):
    with CorpusContext(corpus_name = 'acoustic', **graph_db) as g:
        q = g.query('phone').filter(Annotation.label == 'aa').pitch('praat')
...
def test_query_formants(graph_db):
    with CorpusContext(corpus_name = 'acoustic', **graph_db) as g:
        q = g.query('phone').filter(Annotation.label == 'aa').formants('praat')
