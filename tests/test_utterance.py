...
def test_get_utterances(acoustic_config):
    with CorpusContext(acoustic_config) as g:
        g.reset_utterances()
        g.encode_pauses(["sil"])
        utterances = g.get_utterances(
            "acoustic_corpus", min_pause_length=0, min_utterance_length=0
        )
        ...
        utterances = g.get_utterances(
            "acoustic_corpus", min_pause_length=0.5, min_utterance_length=1.0
        )
        ...
        utterances = g.get_utterances(
            "acoustic_corpus", min_pause_length=0.5, min_utterance_length=1.1
        )
...
def test_encode_utterances(acoustic_config):
    with CorpusContext(acoustic_config) as g:
        ...
        q = g.query_graph(g.utterance)
        q = q.columns(
            g.utterance.begin.column_name("begin"),
...
def test_speech_rate(acoustic_config):
    with CorpusContext(acoustic_config) as g:
        ...
        q = g.query_graph(g.utterance)
        q = q.columns(
            g.utterance.word.rate.column_name("words_per_second"),
            g.utterance.word.label,
        )
        q = q.order_by(g.utterance.begin)
        print(q.cypher())
        results = q.all()
