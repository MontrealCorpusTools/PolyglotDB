...
def load_config():
    ...
    if os.path.exists(CONFIG_PATH):
        ...
        c["Neo4j"] = {"http_port": 7474, "https_port": 7475, "bolt_port": 7687}
...
def download_neo4j(data_directory, overwrite=False):
    ...
    download_link = (
        "https://neo4j.com/artifact.php?name=neo4j-community-{version}-{dist_string}".format(
...
def configure_influxdb(data_directory):
    ...
    with open(influxdb_conf_path, "w") as f:
        f.write(
            template.format(**{k: make_influxdb_safe(v) for k, v in CONFIG["InfluxDB"].items()})
        )
...
def main():
    ...
    install_parser.add_argument(
        "-d", "--download", help="Force download of new versions", action="store_true"
    )
    ...
    if not hasattr(args, "which") or args.which == "help":
        ...
        if os.path.exists(directory):
            if args.quiet:
                ...
                check = input(
                    "The current directory already exists, would you like to overwrite it? (Y/N)"
                ).lower()
                if check == "y":
                    uninstall()
                else:
