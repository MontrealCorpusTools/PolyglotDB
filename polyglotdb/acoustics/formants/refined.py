...
def analyze_formant_points_refinement(
    ...
    """
    if not corpus_context.hierarchy.has_type_subset(
        "phone", vowel_label
    ) and not corpus_context.hierarchy.has_token_subset("phone", vowel_label):
    ...
    for i, ((speaker, vowel), seg) in enumerate(
        ...
        if len(seg) < 6:
            print(
                "Not enough observations of vowel {}, at least 6 are needed, only found {}.".format(
                    vowel, len(seg)
                )
            )
            ...
            for s, data in output.items():
                ...
                for candidate, measurements in data.items():
                    ...
                    for leave_out in range(1, 1 + min(3, candidate)):
                        ...
                        if (
                            leave_out < len(As)
                            and As[leave_out - 1] < intercept + slope * Fs[leave_out - 1]
                        ):
                        ...
                        if this_is_droppable:
                            for parameter in measurements.keys():
                                if int(parameter[-1]) < leave_out:
                                    ...
                                    new_measurements[
                                        parameter[0] + str(int(parameter[-1]) - 1)
        ...
        for s, data in output.items():
            for candidate, measurements in data.items():
                try:
                    output[s][candidate]["A1A2diff"] = (
                        data[candidate]["A1"] - data[candidate]["A2"]
                    )
                    try:
                        output[s][candidate]["A2A3diff"] = (
                            data[candidate]["A2"] - data[candidate]["A3"]
                        )
                    except Exception:
                        try:
        ...
        elif vowel not in vowel_prototype_metadata:
        ...
        if num_iterations > 1 and len(seg) < 6:
            print(
                "Skipping iterations for vowel {}, at least 6 tokens are needed, only found {}.".format(
                    vowel, len(seg)
                )
            )
            my_iterations = [0]
        ...
        for iteration in my_iterations:
            ...
            for s, data in output.items():
                ...
                for number, point in data.items():
                    ...
                    ):  # Update "best" measures when new best distance is found
    ...
    for s, v, token_count, iteration_count in log_output:
        print(
            f"Speaker {s} for vowel {v} had {token_count} tokens and completed refinement in {iteration_count} iterations"
        )
    if output_tracks:
        extract_and_save_formant_tracks(
            corpus_context,
