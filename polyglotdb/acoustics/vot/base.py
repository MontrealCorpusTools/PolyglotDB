...
def analyze_vot(
    ...
    if not corpus_context.hierarchy.has_token_subset(
        "phone", stop_label
    ) and not corpus_context.hierarchy.has_type_subset("phone", stop_label):
    ...
    stop_mapping = generate_segments(
        ...
        padding=PADDING,
        file_type="consonant",
        fetch_subannotations=True,
    ).grouped_mapping("discourse")
    segment_mapping = SegmentMapping()
    vot_func = AutoVOTAnalysisFunction(
        ...
        if (discourse,) in stop_mapping:
            ...
            for speaker in speaker_mapped_stops:
                ...
                segment_mapping.add_file_segment(
                    sf["consonant_file_path"],
                    0,
                    sf["duration"],
                    channel,
                    name="{}-{}".format(speaker, discourse),
                    vot_marks=speaker_mapped_stops[speaker],
                )
    ...
    if already_encoded_vots:
        ...
        updated_data = []
        custom_props = [
            (prop, get_default_for_type(val))
            for prop, val in corpus_context.hierarchy.subannotation_properties["vot"]
