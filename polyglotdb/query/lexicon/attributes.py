from polyglotdb.exceptions import LexiconAttributeError, SubsetError
from polyglotdb.query.base import Node, NodeAttribute
...
class LexiconNode(Node):
    ...
    def __getattr__(self, key):
        if self.hierarchy is not None and not self.hierarchy.has_type_property(
            ...
            properties = [x[0] for x in self.hierarchy.type_properties[self.node_type]]
        ...
        if self.hierarchy is not None:
            for a in args:
                if not self.hierarchy.has_type_subset(self.node_type, a):
                    raise (
                        SubsetError("{} is not a subset of {} types.".format(a, self.node_type))
                    )
