...
from .attributes import LexiconAttribute
...
def generate_match(query):
    return ['({})'.format(query.to_find.define_type_alias)]


def generate_wheres(query):
    properties = []
    for c in query._criterion:
        properties.append(c.for_type_cypher())
    where_string = ''
    if properties:
        where_string += 'WHERE ' + '\nAND '.join(properties)
    return where_string
...
def generate_order_by(query):
    ...
    for c in query._order_by:
        ...
        for col in ac_set:
            if h_c == hash(col):
                element = col.for_cypher(type=True)
                break
        else:
            for col in gb_set:
                if h_c == hash(col):
                ...
                element = c[0].for_cypher(type=True)
                # query.columns(c[0])
        if c[1]:
            element += ' DESC'
...
def generate_distinct(query):
    ...
    properties = []
    for c in query._columns:
        properties.append(c.aliased_for_output(type=True))
    if properties:
        return ', '.join(properties)
    else:
        properties = [query.to_find.type_alias]
...
def generate_return(query):
    ...
    if remove_label_strings:
        if return_statement:
            return_statement += '\nWITH {type_alias}\n'.format(type_alias=query.to_find.type_alias)
...
def query_to_cypher(query):
    ...
    match_strings = generate_match(query)
    kwargs['match'] = 'MATCH ' + ',\n'.join(match_strings)

    kwargs['where'] = generate_wheres(query)
