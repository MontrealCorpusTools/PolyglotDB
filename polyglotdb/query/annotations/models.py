...
from polyglotdb.query.base.helper import value_for_cypher
...
class LinguisticAnnotation(BaseAnnotation):
    ...
    @property
    def waveform(self):
        signal, sr = self.corpus_context.load_waveform(
            self.discourse.name, "low_freq", begin=self.begin, end=self.end
        )
        step = 1 / sr
        data = [
            {"amplitude": float(p), "time": i * step + self.begin} for i, p in enumerate(signal)
        ]
        return data

    @property
    def spectrogram(self):
        orig, time_step, freq_step = self.corpus_context.generate_spectrogram(
            self.discourse.name, "consonant", begin=self.begin, end=self.end
        )
        reshaped = []
            ...
            if key not in self._subs:
                ...
                res = self.corpus_context.execute_cypher(
                    """MATCH (lower_type)<-[:is_a]-(lower_token:{a_type})-[:contained_by]->(token {{id: $id}})
                        RETURN lower_token, lower_type, labels(lower_token) as neo4j_labels ORDER BY lower_token.begin""".format(
                        a_type=key
                    ),
                    id=self._id,
                )
            ...
            if key not in self._supers:
                ...
                res = list(
                    self.corpus_context.execute_cypher(
                        ...
                        RETURN higher_token, higher_type, labels(higher_token) as neo4j_labels""".format(
                            a_type=key
                        ),
                        id=self._id,
                    )
    ...
    def save(self):
        ...
        if self._unsaved:
            ...
            prop_string = ",\n".join(
                ["n.{} = {}".format(k, value_for_cypher(v)) for k, v in props.items()]
            )
            statement = """MATCH (n:{corpus_name}:{type}) WHERE n.id = $id
                        SET {prop_string}""".format(
    ...
    def load(self, id):
        ...
        res = list(
            self.corpus_context.execute_cypher(
                ...
                RETURN token, type, labels(token) as neo4j_labels""",
                id=id,
            )
        )
        labels = res[0]["neo4j_labels"]
    ...
    def delete_subannotation(self, subannotation):
        ...
        for i, sa in enumerate(self._subannotations[subannotation._type]):
            ...
            raise (
                GraphModelError(
                    "Can't delete a subannotation that doesn't belong to this annotation."
                )
            )
        subannotation = self._subannotations[subannotation._type].pop(i)

        statement = """MATCH (n:{type} {{id: $id}}) DETACH DELETE n""".format(
            type=subannotation._type
        )
    ...
    def add_subannotation(self, type, commit=True, transaction=None, **properties):
        ...
        statement = statement.format(
            type=self._type,
            sub_type=type,
            corpus=self.corpus_context.cypher_safe_name,
            props=", ".join(props),
        )
...
class SubAnnotation(BaseAnnotation):
    ...
    def node(self, item):
        ...
        self._id = item["id"]
        if (
            self._annotation._type in self.corpus_context.hierarchy.subannotations
            and self._node["type"]
            in self.corpus_context.hierarchy.subannotations[self._annotation._type]
        ):
    ...
    def load(self, id):
        ...
        res = list(
            self.corpus_context.execute_cypher(
                ...
                RETURN sub, token, type, labels(token) as neo4j_labels, labels(sub) as sub_neo4j_labels""",
                id=id,
            )
        )
        ...
        for x in labels:
            if (
                self._annotation._type in self.corpus_context.hierarchy.subannotations
                and x in self.corpus_context.hierarchy.subannotations[self._annotation._type]
            ):
