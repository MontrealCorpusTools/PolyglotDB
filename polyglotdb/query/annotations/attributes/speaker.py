...
from polyglotdb.query.annotations.attributes.base import Node, NodeAttribute, key_for_cypher
...
class SpeakerAnnotation(Node):
    ...
    def __init__(self, annotation_node):
        super(SpeakerAnnotation, self).__init__(
            "Speaker", annotation_node.corpus, annotation_node.hierarchy
        )
    ...
    def for_match(self):
        kwargs = {"token_alias": self.annotation_node.alias, "alias": self.define_alias}
    ...
    def __getattr__(self, key):
        if self.hierarchy is not None and not self.hierarchy.has_speaker_property(key):
            ...
            raise AnnotationAttributeError(
                f'Speakers do not have a "{key}" property, available are: {", ".join(props)}'
            )
