...
class PrecedesPauseAttribute(NearPauseAttribute):
    def __eq__(self, other):
        ...
        from ..elements import NotPrecedesPauseClauseElement, PrecedesPauseClauseElement
...
class FollowingPauseAnnotation(AnnotationCollectionNode):
    ...
    subquery_match_template = (
        """{collection_alias} = ({anchor_alias})-[:precedes_pause*0..]->(:speech:word)"""
    )
    ...
    def subquery(self, withs, filters=None, optional=False):
        ...
        for_match = self.subquery_match_template.format(
            anchor_alias=self.anchor_node.alias, collection_alias=self.collection_alias
        )
        kwargs = {
            "for_match": for_match,
            "optional": "",
...
class PreviousPauseAnnotation(FollowingPauseAnnotation):
    subquery_match_template = (
        """{collection_alias} = (:speech:word)-[:precedes_pause*0..]->({anchor_alias})"""
    )
...
class PausePathAttribute(AnnotationCollectionAttribute):
    duration_filter_template = (
        "[n in nodes({alias})[-1..]| n.end][0] - [n in nodes({alias})[0..1]| n.begin][0]"
    )
