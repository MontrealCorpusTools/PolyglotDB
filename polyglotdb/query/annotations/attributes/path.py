from polyglotdb.query.annotations.attributes.base import (
    AnnotationCollectionAttribute,
    AnnotationCollectionNode,
    AnnotationNode,
    key_for_cypher,
    special_attributes,
)
...
class SubPathAnnotation(AnnotationCollectionNode):
    ...
    def __repr__(self):
        return "<SubPathAnnotation of {} under {}".format(
    ...
    def generate_subannotation_query(self, input_with_string):
        ...
        return self.subannotation_subquery_template.format(
            def_subannotation_alias=self.def_subannotation_alias,
    ...
    def def_collection_type_alias(self):
        ...
        if self.collected_node.subset_labels:
            subset_type_labels = [
                x
                for x in self.collected_node.subset_labels
                if self.hierarchy.has_type_subset(self.collected_node.node_type, x)
            ]
    ...
    def def_collection_alias(self):
        ...
        if self.collected_node.subset_labels:
            subset_token_labels = [
                x
                for x in self.collected_node.subset_labels
                if self.hierarchy.has_token_subset(self.collected_node.node_type, x)
            ]
    ...
    def __getattr__(self, key):
        ...
        if (
            ...
            and not self.hierarchy.has_token_property(self.collected_node.node_type, key)
            and not self.hierarchy.has_type_property(self.collected_node.node_type, key)
        ):
            raise (
                AttributeError(
                    "The '{}' annotation types do not have a '{}' property.".format(
                        self.collected_node.node_type, key
                    )
                )
            )
...
class PositionalAnnotation(SubPathAnnotation):
    ...
    def __repr__(self):
        return "<PositionalAnnotation object with '{}' type and {} position>".format(
            self.node.node_type, self.pos
        )
...
class PathAttribute(AnnotationCollectionAttribute):
    ...
    position_return_template = (
        "reduce(count = 1, n in [x in {alias} where x.begin < {node_alias}.begin | x] | count + 1)"
    )
    ...
    def for_return(self):
        if self.label == "duration":
            ...
            return self.rate_return_template.format(
                alias=self.node.collection_alias, node_alias=self.node.anchor_node.alias
            )
        elif self.label == "position":
            return self.position_return_template.format(
                alias=self.node.collection_alias,
                node_alias=self.node.collected_node.alias,
            )
        if self.requires_type():
            return self.return_template.format(
                alias=self.node.collection_type_alias, property=self.label
            )
