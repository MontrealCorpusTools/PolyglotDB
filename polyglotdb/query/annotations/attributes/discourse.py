from polyglotdb.query.annotations.attributes.speaker import (
...
class DiscourseAnnotation(SpeakerAnnotation):
    ...
    def __init__(self, annotation_node):
        super(SpeakerAnnotation, self).__init__(
            "Discourse", annotation_node.corpus, annotation_node.hierarchy
        )
    ...
    def __getattr__(self, key):
        if self.hierarchy is not None and not self.hierarchy.has_discourse_property(key):
            ...
            raise AnnotationAttributeError(
                'Discourses do not have a "{}" property, available are: {}.'.format(
                    key, ", ".join(props)
                )
            )
