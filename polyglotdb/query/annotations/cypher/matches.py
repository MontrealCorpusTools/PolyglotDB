from ..attributes import AnnotationNode, PauseAnnotation
...
def generate_match(query, annotation_type, annotation_list, filter_annotations):
    ...
    for a in annotation_list:
        ...
        if a.pos == 0:
            ...
            kwargs = {}
            if isinstance(annotation_type, PauseAnnotation):
                if query.to_find.node_type == query.corpus.word_name:
                    kwargs['node_alias'] = AnnotationNode(query.corpus.word_name, 0, a.corpus).alias  # FIXME?
                else:
                    anno = query.to_find
                    while True:
                        t = query.corpus.hierarchy[anno.node_type]
                        anno = getattr(anno, t)
                ...
                if a.pos + 1 in positions:
                    if query.to_find.node_type != a.node_type:
                        anno = query.to_find
                        while True:
                            t = query.corpus.hierarchy[anno.node_type]
                            anno = getattr(anno, t)
                        ...
                        kwargs['node_alias'] = AnnotationNode(a.node_type, a.pos + 1, a.corpus).alias
                    ...
                    if query.to_find.node_type != a.node_type:
                        ...
                        kwargs['node_alias'] = AnnotationNode(a.node_type, 0, a.corpus).alias
                    if a.pos == -1:
                        kwargs['dist'] = ''
                    else:
        ...
        if a in filter_annotations:
            ...
            if a.node_type not in optionals:
                optionals[a.node_type] = []
            optionals[a.node_type].append((anchor_string, where))
        if isinstance(annotation_type, PauseAnnotation):
