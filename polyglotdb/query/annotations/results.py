from polyglotdb.acoustics.classes import Track
from polyglotdb.exceptions import GraphQueryError
from polyglotdb.query.annotations.attributes import (
    DiscourseAnnotation,
...
from polyglotdb.query.annotations.models import (
    Discourse,
    LinguisticAnnotation,
    Speaker,
    SubAnnotation,
)
from polyglotdb.query.base.results import BaseQueryResults, BaseRecord
...
def hydrate_model(r, to_find, to_find_type, to_preload, to_preload_acoustics, corpus):
        ...
        if isinstance(pre, HierarchicalAnnotation):
            ...
            for i, e in enumerate(subs):
                ...
                for s in subannotations[i]:
                    sa = SubAnnotation(corpus)
                    sa._annotation = pa
                    s["neo4j_label"] = pre.subannotation_alias.split("_in_")[0].replace(
                        "node_", ""
                    )
                    sa.node = s
    ...
    follows = sorted(
        [
            x
            for x in to_preload
            if isinstance(x, FollowingAnnotation) and x.node_type == base_annotation_type
        ],
        key=lambda x: x.pos,
    )
    current = a
    ...
    prevs = sorted(
        [
            x
            for x in to_preload
            if isinstance(x, PreviousAnnotation) and x.node_type == base_annotation_type
        ],
        key=lambda x: x.pos,
        reverse=True,
    )
    ...
    for k, v in a._supers.items():
        follows = sorted(
            [x for x in to_preload if isinstance(x, FollowingAnnotation) and x.node_type == k],
            key=lambda x: x.pos,
        )
        current = v
        for pre in follows:
        ...
        prevs = sorted(
            [x for x in to_preload if isinstance(x, PreviousAnnotation) and x.node_type == k],
            key=lambda x: x.pos,
            reverse=True,
        )
        current = v
    ...
    for pre in to_preload_acoustics:
        ...
        if utterance_id not in pre.attribute.cache:
            data = corpus.get_utterance_acoustics(
                pre.attribute.label, utterance_id, a.discourse.name, a.speaker.name
            )
...
class QueryResults(BaseQueryResults):
    def __init__(self, query):
        ...
        if query._columns:
            ...
            for x in query._acoustic_columns:
                if isinstance(x, TrackAnnotation):
                    self.num_tracks += 1
                    self.track_columns.extend(
                        y for y in x.output_columns if y not in self.track_columns
                    )
                else:
        ...
        if query._columns and self._acoustic_columns:
            ...
            RETURN s.name as speaker, d.name as discourse, r.channel as channel""".format(
                corpus_name=self.corpus.cypher_safe_name
            )
                ...
                if r[a.begin_alias] is None:
                    ...
                    if "utterance" in self.corpus.annotation_types:
                        ...
                        if utterance_id not in a.attribute.cache:
                            data = self.corpus.get_utterance_acoustics(
                                a.attribute.label, utterance_id, discourse, speaker
                            )
                        ...
                        if utterance_id not in a.attribute.cache:
                            data = self.corpus.get_utterance_acoustics(
                                a.attribute.label, None, discourse, speaker
                            )
                            a.attribute.cache[utterance_id] = data
                    t = a.hydrate(self.corpus, utterance_id, r[a.begin_alias], r[a.end_alias])
                    for k in a.output_columns:
                        if k == "time":
                            continue
...
class AnnotationRecord(BaseRecord):
    ...
    def __getitem__(self, key):
        ...
        raise KeyError(
            "{} not in columns {} or {}".format(key, self.columns, self.acoustic_columns)
        )
