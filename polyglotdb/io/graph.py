...
def time_data_to_csvs(type, directory, discourse, timed_data):
    with open(os.path.join(directory, '{}.csv'.format(type)), 'a') as f:
        for t in timed_data:
            f.write('{},{},{}\n'.format(t[0], t[1], discourse))

def import_utterance_csv(corpus_context):
    csv_path = 'file:///{}'.format(os.path.join(corpus_context.config.temporary_directory('csv'), 'utterance.csv').replace('\\','/'))
    statement = '''USING PERIODIC COMMIT 3000
            LOAD CSV FROM "{path}" AS csvLine
            MATCH (begin:word:{corpus} {{begin: toFloat(csvLine[0]), discourse: csvLine[2]}}),
            (end:word:{corpus} {{end: toFloat(csvLine[1]), discourse: csvLine[2]}})
            MERGE (utt:utterance:{corpus}:speech {{begin: toFloat(csvLine[0]), end: toFloat(csvLine[1]), discourse: csvLine[2]}})-[:is_a]->(u_type:utterance_type)
            WITH utt, begin, end
